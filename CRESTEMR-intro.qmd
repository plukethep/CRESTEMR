...
2. Using the TIMSS dataset (see @sec-TIMSS), plot a graph to help your work out whether there is a relationship between _School Emphasis on Academic Success_ `BCBGEAS` and _School Discipline_ `BCBGDAS`. Give the graph sensible x and y labels (e.g. `xlab`). **HINT**: you might need to turn the x and y values into numbers using `as.numeric()`. More details [here](https://timssandpirls.bc.edu/timss2019/methods/pdf/T19_MP_Ch16-context-questionnaire-scales.pdf).
...
`ifelse` allow us to _recode_ the data. In the example below, we are going to add a new column to the schools table (using `mutate`) noting whether a school is a `grammar` school or not. A school is a grammar school _if_ it is not an _Independent_ school _and_ it is _selective_ in its admissions policy, either of these two criteria being false will mean that the school is not a grammar school. 
...
- 4 uses mutate to create a new column in the schools table called `grammar`, this will be given the value of the `ifelse` statement calculation for each line.
...
- 7 is the value that will be returned `if` the statements on line 5 and 6 are both _TRUE_
- 8 is the value that will be returned if the combined statements on line 5 and 6 are _FALSE_, i.e. _else_ the values on line 5 and 6 are true
...
- 4 `colour` is also inside `aes()` and takes the newly coded `grammar` values, this means that grammar schools will be a different colour from non-grammar schools
...
ggplot(data=schools %>% filter(Phase == "Secondary"), 
       x=Region +
  geom_bar(aes(fill=Gender) position="full") # missing comma # 4 position="full" rather than fill 
```
...
ggplot(data=plot_data,
       aes(x=factor(BCBG14G, levels = c("Very low", "Low", "Medium", "High", "Very high"))
           )) +
  geom_bar() +
  facet_wrap(.~CNTRY)
...
ggplot(data=plot_data,
  ...
  geom_bar(aes(fill=CNTRY))

ggplot(data=plot_data,
       aes(x=BCBG14G
           )) +
  geom_bar(aes(fill=CNTRY), position="dodge") # , stat="percentage"
...
## Exporting plots
`ggsave(<file_name_and_extension>, <object_type>)`
