...
Before we start graphing we need to look at how we can _recode_ values. Recoding is very useful when you have a column with multiple items that you want to change to just a few. A common way to recode values is through:
...
::: callout-important
If you define the `colour`, `size` and `alpha` attributes _outside_ the `aes()` function you will hard code the values and they won't change when your dataset changes. Placing these attributes inside `aes` allows them to be dynamically changed by your dataset
:::
