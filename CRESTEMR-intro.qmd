...
## Setting up RStudio and the tidyverse
...
This section will look at how R stores data in *dataframes* (also known as _tibble_, the equivalent of tables in a spreadsheet, ), and how we can access columns and rows from these dataframes. First we need to get some data into R so we can start analysing them. For this example we are going to be using GCSE results for English schools in 2018/19[^4].
...
-   To find the location of a file in *Mac OSX* do the following:
    ...
    3. Right click on the file name and select `Copy <name of file> as Pathname`
...
When you load the tables you will get some warnings:
...
Alternatively, click on the `Environment panel` and the table icon ![](images/icon_table.png) to explore each table:
...
::: callout-tip
Check your data for `NA`s before running calculations. The `na.rm = TRUE` can be applied to most mathematical functions
:::
...
setwd("C:\Users/Peter/Google Drive/Kings/R intro/code")
results <- read_excel("testdata.xlsx", "Results)
schools < read_excel("testdata.xlsx", "Schools")
...
`summarise(<col_name> = <calculation>)` performs a calculation on a given grouping, returning the result but dropping all the columns except for the result and the given *grouping* columns.
...
1.  gets the `results` dataframe and *pipes* it into the next instruction, whatever results from all the piping will be stored `<-` into the `sub_entries` object

2.  groups the results by individual *school* (`ID`) *and* by *subject* taken in that school (`Description`).

3.  creates a new column called `Total` and stores the *maximum* value from the `Entries` column, i.e. the total number entered for each subject. As the piped data is currently grouped, this will work out the total number entered for every school and subject combination.

4.  as the pipe currently stores the total number of students entered for each subject in each school, we now group on `Description` *only* to allow us to look at individual subjects across *all* schools

5.  creates a new column called `Subject_Total` and stores the sum of all the `Total` in each `Description` grouping, i.e. the number of students taking each subject for the whole of England
...
3.  `filter` returns only those rows that have "Total number entered" in their `Grade` column, i.e. it filters out all of the individual grade numbers.

4.  `select` gets rid of all the columns other than those listed.
...
Very often when dealing with datasets such as TIMSS or PISA, the column names can be very confusing without a reference key, e.g. _BCBG10B_ and _BCBG11._ To rename columns in the tidyverse we use the `rename(<old_name> = <new_name>)` command. For example, if you wanted to rename the rather confusing schools column called `Open` you would do the following:
...
The easiest way to do this in R is to use the `write.csv(<table>, <location>)` function. This function lets you specify the name of the table object you want to save, along with where you want to save it.:

```{r}
#| eval: false

data_output <- data_joined %>% 
  filter(Description == "Chinese", 
         Grade == "Total number entered" ) %>% 
  select(ID, Total_students, Description, 
         Entries, FSM, NumberOfBoys, NumberOfGirls)

write.csv(data_output, "file location")
```

::: callout-tip
You can write excel files using the `openxlsx` package, more information [here](https://www.rdocumentation.org/packages/openxlsx/versions/4.2.5)
:::
...
You can access the cut down TIMSS school dataset as an Excel file [here](https://docs.google.com/spreadsheets/d/1Sgyw1tLbPGsl4HeyhpNGLhJwTNIriE-B/edit?usp=sharing&ouid=105471590997842250248). Save it to your computer and load it into an object called `TIMSS` using the `read_excel(<location_of_file>, <sheet_name>)` command:
...
For help on loading data see @sec-loading.

Take a look at the data using the Environment panel, it's rather confusing as Excel didn't store the full question names. This data originally comes from SPSS and it is possible to look at the names in R, but here we might need to do some renaming to make the dataset a little more manageable (see @sec-renaming).
...
3. For the students in each country: What is the average and median instructional time in hours, in a typical day? Can you arrange the results so we find the hardest working country? You need to use `BCBG06B`, and hint, this column might not be `as.numeric()` just yet. (You might want to use `mutate` and @sec-datatypes to help you)
...
computing_test <- data_joined %>%
  filter(grepl("Comput", Description),
         ...
         Grade == "Total number entered" ) %>%
  select(ID, Total_students, Entries, 
...
- 18-19 these lines set up the `ggplot` giving it the table object `computing_test` as its data input and setting up the aesthetics for the rest of the graph elements using columns from `computing_test`. The `aes(<attribute>, <attribute>, ...)` command allows us to specify aesthetic elements of the graph that will change dependent on the dataset we use. `x=per_male` and `y=per_taking_cs` define the x and y coordinates, defining `aes()` in ggplot means we will pass down these values to the subsequent geometric objects so we don't have to define these `x` and `y` axis items again.
- 20 using the data and `aes` values defined on lines 18-19, `geom_point` uses the set `x` and `y` values to draw a point for each school in our dataset. There are lots of different parameters we could give `geom_point`, but here we are content with using the defaults.
- 21 we add another geometric object on top of points, this time we add a line of best fit `geom_smooth`, again this geometric object uses the values specified on lines 18-19, and we define the `method` as `lm`, to calculate a linear model line of best fit.
...
`ifelse` allow us to _recode_ the data. For the example below, we are going to add a new column to the schools table noting whether a school is a `grammar` school or not. A school is a grammar school _if_ it is not an _Independent_ school _and_ it is _selective_ in its admissions policy, either of these two criteria being false will mean that the school is not a grammar school. 

You can see how the above if statement is implemented in R on lines 4-7.
...
- 5 the `if` statement to be evaluated as `true` or `false` has two parts, separated by the ampersand symbol `&` instead of the word _and_ (as we have seen above, _or_ is implemented using a bar `|`).
- 6 is the value that will be returned if the above statement(s) are _true_
- 7 is the value that will be returned if the above statement(s) are _false_
...
let's move on to looking at `geom_` elements:

### Geom_point
Rather unsurprisingly, `geom_point` allows us to plot a layer of points using x and y coordinates, as shown in the example above. We are now going to look at how `geom_point`s can be adapted to put more data into our graphs by taking the `schools` dataset and using the `colour` and `size` attributes to display the population and nature of schools. 

Now we have the data recoded to display whether a school is a grammar school or not, we can plot this dataset onto the graph:
...
- 2 we define the `x` and `y` `aes`theic values here, we could have done this on line one, but this shows that this can be done separately for each `geom_`
...
- 6 to stop the schools blotting each other out, we set the `alpha` (transparency) of each point to `0.4`. This is done outside the aes as we want all points to have the same transparency.

::: callout-important
If you define the `colour`, `size` and `alpha` attributes _outside_ the `aes()` function you will hard code the values and they won't change with your dataset. Placing these attributes inside aes allows them to be dynamically changed by your dataset
:::
...
1. Errors in this graph code

2. ifelse question

1. Plot the size of an open primary `school` against the `FSM` grade. Are larger primary schools generally serving poorer communities?
...
2. plot involving size and colour

3. plot involving ifelse

:::

Adding a map to demonstrate layering
...
#filter to select only the maths results
top_maths <- top_grades %>% filter(Description == "Mathematics")

# Build the model
result_maths <- lm(per_top ~ FSM, top_maths)
