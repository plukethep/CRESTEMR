...
2. Using the TIMSS dataset (see @sec-TIMSS), plot a graph to help your work out whether there is a relationship between _School Emphasis on Academic Success_ `BCBGEAS` and _School Discipline_ `BCBGDAS`. Give the graph sensible x and y labels (e.g. `xlab`). More details [here](https://timssandpirls.bc.edu/timss2019/methods/pdf/T19_MP_Ch16-context-questionnaire-scales.pdf).
...
, as shown in the example above. We are now going to look at how `geom_point`s can be adapted to put more data into our graphs by taking the `schools` dataset and using the `colour` and `size` attributes to display the population and nature of schools.
...
- 5 the `if` statement to be evaluated as either `true` or `false` has two parts, separated by the ampersand symbol `&` instead of the word _and_ (as we have seen above, _or_ is implemented using a bar `|`).
- 6 is the value that will be returned if the statements on line 5 are _TRUE_
- 7 is the value that will be returned if the statements on line 5 are _FALSE_
...
Let's return to the dataset, `computing_test`, we had built in @sec-graphing. We might speculate that it is the boys only schools influencing the data that  in a school predicting 

```{r}

# unique(data_joined$EstablishmentGroup)
# 
# ggplot(data=computing_test, 
#        aes(x=per_male, y=per_taking_cs)) +
#   geom_point() +
#   geom_smooth(method='lm') +
#   ggtitle("The more boys, the more CS") +
#   facet_grid(Gender ~ EstablishmentGroup)
# 
# 
# # grepl looks for the word “Comput” in the string Description
# computing_test <- data_joined %>%
...
#          FSM, NumberOfBoys, NumberOfGirls, EstablishmentGroup, Gender) %>%
#   mutate(per_male =
#            NumberOfBoys / (NumberOfBoys + NumberOfGirls),
#         per_taking_cs = Entries/Total_students) %>%
# arrange(desc(per_taking_cs))
...
# Build the model
result_maths <- lm(per_top ~ FSM, data=top_maths)
